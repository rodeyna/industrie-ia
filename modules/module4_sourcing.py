...
if __name__ == "__main__":
    ...
    # ADD this at the bottom of real_data_sourcing.py
# (keep ALL her existing code exactly as is, just add this function)

def run_module4(state: dict) -> dict:
    print("\n[M4] Running sourcing...")
    
    specs = state.get("specs") or {}
    
    # Get material from M1 specs
    # Her code uses "316 S/S" format, M1 gives "Stainless Steel 316L"
    # so we save requirements.json that her code reads
    import json, os
    requirements = {
        "material": specs.get("materials", {}).get("ball_stem", "316 S/S"),
        "DN": specs.get("selected_model_specs", {}).get("dn_size", 50)
    }
    
    # Save requirements.json so her run_module_4() can read it
    script_dir = os.path.dirname(os.path.abspath(__file__))
    with open(os.path.join(script_dir, "requirements.json"), "w") as f:
        json.dump(requirements, f)
    
    # Run her original function
    run_module_4()
    
    # Read her output back into state
    suppliers_file = os.path.join(script_dir, "suppliers_data.json")
    try:
    ...
    return {**state, "suppliers": suppliers, "supplier_data": suppliers}